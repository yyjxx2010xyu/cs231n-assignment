...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        relu_out1, relu_cache1 = relu_forward(out1)
        
        out2, cache2 = affine_forward(relu_out1, self.params["W2"], self.params["b2"])
        relu_out2, relu_cache2 = relu_forward(out2)
        ...
        dout2 = relu_backward(dout, relu_cache2)
        dX2, dW2, db2 = affine_backward(dout2, cache2)
        
        dout1 = np.dot(dout2, self.params["W2"].T)
        dX1, dW1, db1 = affine_backward(dout1, cache1)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
          if self.use_batchnorm:
            next_layer, cache[i] = batchnorm_forward(cur_layer,
                                                          self.params["gamma%d" % (i+1)],
                                                          self.params["beta%d" % (i+1)],
                                                          self.bn_params[i])
          else:
            next_layer, cache[i] = affine_relu_forward(cur_layer,
                                                       self.params["W%d" % (i+1)],
                                                       self.params["b%d" % (i+1)])
          if self.use_dropout:
            next_layer, drop_cache[i] = dropout_forward(cur_layer, 
                                                   self.dropout_param)
          cur_layer = next_layer  
          
        out, cache[self.num_layers - 1] = affine_forward(cur_layer,
        ...
        dnext_layer, dW, db = affine_backward(dout, cache[self.num_layers - 1])
        ...
        for i in range(self.num_layers - 1, 0 , -1):
          ...
          if self.use_dropout:
            dcur_layer = dropout_backward(dnext_layer, drop_cache[i-1])
          if self.use_batchnorm:
            dcur_layer, dW, db, dgamma, dbeta = batchnorm_backward(dnext_layer, cache[i-1])
            grads["gamma%d" % i] = dgamma
            grads["beta%d" % i] = dbeta
          else:
            dcur_layer, dW, db = affine_relu_backward(dnext_layer, cache[i-1])
          dnext_layer = dcur_layer
